 ...
 * @c1: first spot that was checked
 * @c2: first spot in wildcard comparison
 *
 * Return: spot that matches or NULL
 */
char *test(char *s5, char *s6, char *c1, char *c2)
{
	if (*s6 == '*' || (*s5 == '\0' && *s6 == '\0'))
		return (c1);
	else if (*s5 != *s6)
		return (wildcard(s5, c2));
	else
		return (test(s5 + 1, s6 + 1, c1, c2));
}
